...
def _send_business_recent_followups_2h(limit=8):
    ...
    try:
        ...
        kb = [
            [{"text": "🔔 Updates & Quiz", "url": v83.PREFERENCES_DEEPLINK}],
            [{"text": "🎁 Promotions", "url": "https://t.me/BetroxyBot/promotions"}],
            [{"text": "🎧 Support", "url": v83.SUPPORT_URL}],
        ]
        text = (
            "👋 <b>Need anything else?</b>\n\n"
            "If you still need help, reply here anytime. You can also choose BETROXY "
            "updates & quizzes, view Promotions, or contact support below."
        )
        ...
        if rows:
            bot.logger.warning(
                "BUSINESS_FOLLOWUP_2H_SCAN eligible=%s sent=%s open_only=on media=text_only",
                len(rows), sent,
            )
        return sent
    except Exception:
